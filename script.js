...
function sortAToZ(){
    let updatedStudents = students.sort((a,b)=> {
...
function sortByPassing(){
    let updatedStudents = students.filter( (item)=> item.passing );
...
function sortByGender(){
    let maleStudents = students.filter( (item) => item.gender=="Male")
    let femaleStudents = students.filter( (item) => item.gender=="Female")

    // console.log("male table", maleStudents, maleStudents.length)
    // console.log("female table", femaleStudents, femaleStudents.length)

    // renderTwoTables(maleStudents, femaleStudents);
    

    tablebodyCol.innerHTML = ""
    maleStudents.map( (item)=>{
        tablebodyCol.innerHTML += `
            <tr>
                <td>${item.id}</td>
                <td><img src=${item.img_src} />  ${item.first_name} ${item.last_name}</td>
...
function searching(){
    let name = document.getElementsByName("search")[0].value.toLowerCase();
    // console.log(name.toLowerCase())
    let updatedStudents = students.filter( (item) => {
        if(item.first_name.toLowerCase() === name || item.last_name.toLowerCase()=== name || item.email.toLowerCase() === name){
            return item;
        }
    } );
